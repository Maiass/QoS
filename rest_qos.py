...
#here I added new variable for getting and setting hard_timeout
REST_HARD_TIMEOUT = 'hard_timeout'
...
class QoS(object):
    ...
    def set_default_flow(self):
		...
		#here I invoke _to_of_flow with hardtimeout variable whose value is 0 by default for unlimited live time rule in table 0
    ...
    def _set_qos(self, cookie, rest, waiters, vlan_id):
		...
		#get hard_timeout'value from rest api as entered from app layer
        hard_timeout = rest.get(REST_HARD_TIMEOUT)
        ...
        actions.append({'type': 'GOTO_TABLE',
                        'table_id': QOS_TABLE_ID + 1})
		#invoke _to_of_flow with hard_timeout varaible with its entered value from APP layer
        flow = self._to_of_flow(cookie=cookie, priority=priority,
                                match=match, actions=actions, hardtimeout=hard_timeout)
    ...
    def delete_qos(self, rest, vlan_id, waiters):
        ...
        if str(self.dp.id) in msgs:
            ...
            for flow_stat in flow_stats:
                ...
                priority = flow_stat[REST_PRIORITY]
				#get hard_timeout values 
                hard_timeout = flow_stat[REST_HARD_TIMEOUT]
                ...
                if priority != DEFAULT_FLOW_PRIORITY:
                    if ((qos_id == REST_ALL or qos_id == ruleid) and
						...
						#append hard_timeout values to list
                        delete_list.append([cookie, priority, match, hard_timeout])
                    else:
        ...
        if len(delete_list) == 0:
            ...
            actions = []
            delete_ids = {}
			#for each value in the list including the hard_timeout
            for cookie, priority, match, hard_timeout in delete_list:
                flow = self._to_of_flow(cookie=cookie, priority=priority,
                                        match=match, actions=actions, hardtimeout=hard_timeout)
                self.ofctl.mod_flow_entry(self.dp, flow, cmd)
    ...
    def _to_of_flow(self, cookie, priority, match, actions, hardtimeout):
        flow = {'cookie': cookie,
                'priority': priority,
                'flags': 0,
                'idle_timeout': 0,
